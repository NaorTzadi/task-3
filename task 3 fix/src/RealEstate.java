...
public class RealEstate {
                    ...
                    if (Type == 3|| Type ==2 || Type == 1) {
                        ...
                        if (rooms.matches("\\d+")) {
                            ...
                            if (numberOfProperty.matches("\\d+")) {
                                ...
                                String saleOrRent = scanner.next();
                                boolean rentOrSale=false;
                                if (saleOrRent.equals("rent") || saleOrRent.equals("sale")){
                                    ...
                                    if (price.matches("\\d+")) {
                                        ...
                                        System.out.println("property uploaded");
                                        System.out.println();
                                        SubMenu(position,name,realEstate);
                                    }
                                }
    ...
    public static void removeProperty(int position,String name,RealEstate realEstate){
        ...
        for (int i = 0; i < realEstate.PROPERTIES.length-1; i++) {
            if (realEstate.PROPERTIES[i].getNAME_OF_SELLER().equals(name)){
                ...
                System.out.println(realEstate.PROPERTIES[i].getCITY_OF_PROPERTY()+"-"
                        +realEstate.PROPERTIES[i].getSTREET_OF_PROPERTY()+" "+realEstate.PROPERTIES[i].getNUMBER_OF_PROPERTY());
                System.out.println(realEstate.PROPERTIES[i].getTYPE_OF_PROPERTY()+"-"+realEstate.PROPERTIES[i].getIF_FOR_SALE()+": "+
                        realEstate.PROPERTIES[i].getNUMBER_OF_ROOMS()+", "+
                        realEstate.PROPERTIES[i].getFLAT_OF_PROPERTY()+".");
                System.out.println("price: "+realEstate.PROPERTIES[i].getPRICE_OF_PROPERTY()+"$.");
                System.out.println("contact info: "+User.userNames[position]+" "+User.phoneNumber[position]+"("+User.sellerOrUser[position]+")");
            }
        }
        Scanner scanner=new Scanner(System.in);
    ...
    //לפי התוכנה זה שבריר של כפל קוד ולא כפל קוד אבל נראה לי שעם כמות השברים שיש לנו אפשר כבר להרכיב כפל קוד (:
    public static void printProperties(int position,String name,RealEstate realEstate) {
        ...
        for (int i = 0; i < realEstate.PROPERTIES.length-1; i++) {
            if (realEstate.PROPERTIES[i].getNAME_OF_SELLER().equals(name)){
                System.out.println(realEstate.PROPERTIES[i].getCITY_OF_PROPERTY()+"-"
                        +realEstate.PROPERTIES[i].getSTREET_OF_PROPERTY()+" "+realEstate.PROPERTIES[i].getNUMBER_OF_PROPERTY());
                System.out.println(realEstate.PROPERTIES[i].getTYPE_OF_PROPERTY()+"-"+realEstate.PROPERTIES[i].getIF_FOR_SALE()+": "+realEstate.PROPERTIES[i].getNUMBER_OF_ROOMS()+", "+
                        realEstate.PROPERTIES[i].getFLAT_OF_PROPERTY()+".");
                System.out.println("price: "+realEstate.PROPERTIES[i].getPRICE_OF_PROPERTY()+"$.");
                System.out.println("contact info: "+User.userNames[position]+" "+User.phoneNumber[position]+"("+User.sellerOrUser[position]+")");
            }
        ...
        String back;
        do{
            System.out.println("type 1 to go back to the sub menu.");
           back=scanner.next();
            SubMenu(position, name, realEstate);
        }while(back.equals("1"));
    }
